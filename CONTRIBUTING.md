...
Optional flags. Toggles are off by default. `CORPUS_RETRIEVAL_MODE` defaults to `dual` and `RECEIPT_DELIVERY_MODE` to `auto`. Unset `CHECKPOINTER` means Postgres whenever `DATABASE_URL` is set.
