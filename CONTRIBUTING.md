...
- `CURRENCY_CHECK_ENABLED=on` — adds a `currency_check` node between `grounding_check` and `supervisor`. Off by default, fails open: any error returns no labels for that turn, never blocks the answer. Makes no network call and no model call. For each cited Act, reads its own metadata timeline (`data/acts_metadata/<act>.json`) for a `REPEALED`/`SUPERSEDED` entry, attaching a Currency Label to the citation. Ships on the SSE `response` event and `QueryResult` as `currency_labels`, present only when non-empty. The frontend renders it as a badge on the citation (`frontend/components/locus-workspace/Messages.tsx`). See [CONTEXT.md](CONTEXT.md#language)'s **Currency Label** entry for what counts as a hit and the two limits an operator should hold in mind before trusting an `unknown` result.
