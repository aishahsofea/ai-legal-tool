...
- `SEMANTIC_MEMORY_PRUNE=on` — background **maintenance** path (`agent/memory/pruner.py`). Consolidates duplicate profiles / near-duplicate topics, evicts low-value topics by importance + recency, not TTL. Off by default, fail-open, off the hot path, size-debounced. Conservative: never deletes the sole profile, never empties a namespace.
- `AGENTIC_RETRIEVAL=1` — swaps the deterministic `retriever` node for a `create_agent` ReAct loop that binds `search_statutes` / `lookup_section` and decides how to search (ADR 0013). Accepts `1`, `true`, or `yes`. Off by default, fail-open — any error or empty result falls back to the deterministic pgvector path. On: the retry loop re-retrieves with feedback on an evidence violation instead of only re-drafting, and retrieval tools stream `tool_call` SSE events into the PROCESS panel. The eval `tool_selection` assertion (`expected_tool`) only activates with this flag on. `RETRIEVAL_RECURSION_LIMIT` (default 6) bounds the ReAct loop.
- `CORPUS_RETRIEVAL_MODE=dual|verified|legacy` — `dual` (default) reads legacy rows plus provenance rows joined to the active Act/language mapping. `verified` reads active provenance only. `legacy` is the rollback path, excludes shadow rows.
- `RECEIPT_DELIVERY_MODE=auto|local|redirect|proxy` — `auto` uses verified local bytes when present, else CDN objects whose length, content type, and `x-amz-meta-sha256` match the registry. Remote coordinate sidecars get hash-checked again after download. `redirect` and `proxy` both need `CORPUS_CDN_BASE_URL`.
- `REFERENCE_GRAPH_ENABLED=on` — exposes a **promoted**, independently validated statutory reference graph. Off by default; alone, builds/promotes/loads nothing. `REFERENCE_GRAPH_ROOT` can point at a read-only promoted-artifact root for an operator deployment.
- `REFERENCE_GRAPH_COMPARISON_ENABLED=on` — adds snapshot selection and one-hop comparison, only while the base flag is also on. Independently off by default, fails closed without disabling Phase 1.
- `FOLLOW_REFERENCES_ENABLED=on` — adds `follow_references` to the **Retrieval Agent** only, so `AGENTIC_RETRIEVAL=1` is also required. Independently off by default; doesn't need `REFERENCE_GRAPH_ENABLED` — internal retrieval reads the same promoted artifacts directly through `ReferenceGraphStore`, while public UI/API exposure is controlled separately. Flag off → model sees the original two tools, original prompt. `REFERENCE_GRAPH_ROOT` controls the read-only artifact root for both uses.
