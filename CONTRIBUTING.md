...
- `AGENTIC_RETRIEVAL=1` — swaps the deterministic `retriever` node for a `create_agent` ReAct loop that binds `search_statutes` / `lookup_section` and decides how to search (ADR 0013). Accepts `1`, `true`, or `yes`. Off by default, fail-open — any error or empty result falls back to the deterministic pgvector path. When on:
