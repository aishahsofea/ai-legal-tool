...
- **Receipt Documents use content-derived identities.** A registered PDF is identified as `act-<act>-<language>-sha256-<fullhash>` and stored under a SHA-256 asset key. The registry records its hash, byte size, page count, original source URL, timeline metadata, scrape timestamp, source language, and lifecycle status. A changed hash creates a new document; it never overwrites or reassigns the old identity.
...
- **Corpus lifecycle operations use an operator CLI, `python -m corpus`.** It covers manifest generation, deep validation, shadow extraction, migration, registration, ingestion, upload, activation, and rollback. These are offline/batch control-plane operations: large, credentialed, and deliberately reviewed. Exposing them as runtime HTTP endpoints would unnecessarily expand the production attack surface. State-changing commands support dry-run or explicit per-identity inputs.
...
- The initial audit is frozen at 624 input PDFs:
  - 596 canonical reprints registered
  - 576 extraction identities ready
  - five pilots active
  - 48 blocked: 28 amendment-only, 15 no-chunk, 5 scanned/image-only

  The six BM-only sources remain labeled `bm`.
