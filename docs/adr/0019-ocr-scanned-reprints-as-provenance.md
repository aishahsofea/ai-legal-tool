...
- **Every OCR input that can change output joins `EXTRACTOR_CONFIG`.** Render DPI, the Tesseract language string per document language, and the PyMuPDF version. `pdfocr_tobytes`/`get_textpage_ocr` run MuPDF's own statically-linked Tesseract and Leptonica, not the system `tesseract` binary — PyMuPDF exposes no runtime version for either engine, so the PyMuPDF version is the queryable proxy: it fixes which OCR build is compiled in. Skipping it would let a PyMuPDF upgrade on a future machine silently change a scanned Act's chunk text under an extraction identity that claims to be unchanged. `EXTRACTOR_VERSION` bumps in the same change; Consequences covers what that means for the rest of the corpus.
...
**Determinism.** Three separate processes OCR-ing the same page of Act 627 produced the identical word list (same SHA-256). Two separate processes extracting all of Act 150 produced the identical `chunk_set_hash`. This ran through PyMuPDF's `pdfocr_tobytes`/`get_textpage_ocr`, which use MuPDF's own bundled Tesseract and Leptonica — a different binary from the system `tesseract` command. An earlier pass of this evidence recorded the system `tesseract --version` (5.5.2) as the engine in use; it wasn't. MuPDF's own build scripts vendor Tesseract 5.0.0, confirmed by inspecting the compiled library — PyMuPDF exposes no runtime Leptonica version to check the same way. Confirmed on one machine only — the `pymupdf_version` key in `EXTRACTOR_CONFIG` is what would turn a second machine's drift, if there is any, into a new identity instead of silent divergence.
