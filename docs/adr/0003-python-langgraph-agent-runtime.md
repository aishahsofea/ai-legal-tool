...
The project owner is a Senior Frontend Engineer with a TypeScript background. But the agent engineering skills this project needs to demonstrate for hiring — tool orchestration, state machines, supervision layers, eval harnesses — are substantially better supported in the Python ecosystem. LangGraph, LangSmith, and the broader LangChain tooling are all Python-first. Running agent logic in TypeScript (Mastra/Vercel AI SDK) instead would fragment tracing and evals across two runtimes, and lose the depth of LangGraph's graph primitives.
