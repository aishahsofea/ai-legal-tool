...
- **Clarification is a dedicated `clarify` node reached by a new router branch.** The router's existing structured-output call gains a `"clarify"` `query_type` and a `clarifying_question` field, so detection costs **zero extra latency** on the normal path. `_route_from_router` routes to `clarify` only when all three hold: the query is `clarify`, a non-empty question was produced, and the turn has not already asked one. Otherwise — an empty question, or a second ambiguity — it falls through to the normal legal path, avoiding both pausing on nothing and looping.
