...
- Resumable: skips acts whose file already has both languages recorded and dated. An Act scraped before the BM fetch existed gets its BM fields backfilled on the next run, using the current listing's stored Malay link. Its existing primary fields are left untouched
- Each language carries its own scrape date (`scraped_at`, `scraped_at_bm`), because the Malay side is usually fetched in a later run than the primary. A Malay document recorded before `scraped_at_bm` existed has no date of its own, so it is re-fetched once to get one. Without that, Step 3 files it under the English scrape's date (issue #71). One-off cost: 879 Acts are in this state, 678 of which need a request (the other 201 are the no-link case below)
- If the listing no longer carries a Malay link for an Act that already has a Malay document on file, the document is kept and dated from `scraped_at`. A link that disappeared is not evidence the document never existed. 201 Acts are in this state
- `python run.py --act N --html saved.html` parses the English page offline and never touches the Malay side. It leaves the `_bm` fields unset, not empty — empty is the confirmed-absent record below, and would retire the Act from backfill for good
