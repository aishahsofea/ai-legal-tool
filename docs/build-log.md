...
**2026-09-19** — #56 Phase 3 (`search_commentary` tool): the plan's own Files list for this phase (`docs/commentary-implementation-plan.md`) named `agent/retrieval/tools.py`, `agent/retrieval/agent.py`, and their two test files, but not `agent/nodes/retriever.py`. Built exactly as scoped, the tool would have written its results into the ReAct sub-agent's own `commentary` channel and then lost them at the node boundary: `agentic_retriever_node` builds its LangGraph return dict by naming specific keys off `run_retrieval_agent`'s result (`chunks`, `tools`, then `reference_trace`/`reference_metrics` conditionally) rather than passing the result through, so an unlisted key is silently dropped, not an error. `AgentState.commentary`, reset to `[]` every turn in `_start_turn`, would have stayed `[]` forever with the flag on. No test at this phase's own scope would have caught it — every existing test patches `run_retrieval_agent` directly and asserts on its return value, never on what reaches `AgentState`. Threaded `commentary` through `agentic_retriever_node` the same way `reference_trace`/`reference_metrics` already are, in both its success and its fail-open-to-deterministic branches.
