...
def _rectangles(pdf: fitz.Document, matched: list[_SourceToken]) -> list[LocatedPage]:
    ...
    for (page_number, _block, _line), line_words in sorted(lines.items()):
        ...
        pages.setdefault(page_number, []).append(Rectangle(
            x=bounded((x0 - page_rect.x0) / page_rect.width),
            y=bounded((y0 - page_rect.y0) / page_rect.height),
            width=bounded((x1 - x0) / page_rect.width),
            height=bounded((y1 - y0) / page_rect.height),
        ))
    return [LocatedPage(page_number, rectangles) for page_number, rectangles in sorted(pages.items())]


def locate_evidence(path: Path, evidence_quote: str | None, start_page: int) -> LocatorResult:
    with fitz.open(path) as pdf:
        ...
        for token in _source_tokens(_extract_words(pdf, start_page)):
            window.append(token)
        ...
        return LocatorResult("matched", start_page, _rectangles(pdf, nearest_matches[0]))
